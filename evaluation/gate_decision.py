...
def main() -> None:
    ...
    decision = decide_gate(comparison, ablation)
    save_json(args.output, decision)
    print(json.dumps(decision, indent=2, sort_keys=True))


def decide_gate(comparison: dict[str, Any], ablation: dict[str, Any]) -> dict[str, Any]:
    ...
    status = classify_gate(checks, risks, deltas)
    if status == "pass":
...
def classify_gate(checks: dict[str, Any], risks: list[str], deltas: dict[str, Any]) -> GateStatus:
    required_positive = [
