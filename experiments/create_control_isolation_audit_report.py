"""Create the GeoAttention v2 control-isolation audit report."""
...
from evaluation.control_isolation_audit import (  # noqa: E402
    build_control_isolation_audit_report,
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create ERGT GeoAttention v2 control-isolation audit report."
    )
    parser.add_argument(
        "--config",
        type=Path,
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("runs/geoattention_v2/control_isolation_audit_report.json"),
        help="Output path for control_isolation_audit_report.json.",
    )
...
def main() -> None:
    ...
    report = build_control_isolation_audit_report(
